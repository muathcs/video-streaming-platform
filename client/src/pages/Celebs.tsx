...
function Celebs() {
  ...
  // ps: the server dictates how many celebs to fetch, if it's not mentioned in the payload. Default is 10. 
    ...
    const fetchRecommendations = async () => {
      if (userInfo?.fav_categories) {
        try {
          const response = await axios.get(`${apiUrl}/celebs/rec/${userInfo.fav_categories}`);
  ...
  return (
      <div className=" flex flex-col w-full items-center gap-10  gradiant-page">
        ...
        <ShopByCat/>
        <FeaturePicks/>
        <Recommendations fav_categories={userInfo?.fav_categories} recommendations={recommendations}/>
...
function ShopByCat(){
  const [amountOfCategoryToShow, setAmountOfCategoryToShow] = useState<number>(6)
...
function Recommendations({fav_categories, recommendations}:{fav_categories:string | undefined, recommendations:CelebType[]}){
  const navigate = useNavigate()
  return (
    <>
            {/* Recommendations */}
