...
function Dashboard() {
  ...
  const columns: Column<PartialRequestType>[] = React.useMemo(
    () => [
      ...
      {
        Header: "Reply",
        Cell: ({ row }: { row: { original: PartialRequestType } }) => (
          <Button
            onClick={() => handleFulfillReq(row.original)}
            variant="outline"
      ...
      },
      {
        Header: "Time Left",
        Cell: ({ row }: { row: { original: PartialRequestType } }) => {
          const timeLeft = calculateTimeLeftForCelebrityReply(
                  ...
                  <tr {...headerGroup.getHeaderGroupProps()} className="">
                    {headerGroup.headers.map((column) => (
                      <th
                        ...
                        className="p-2 border border-gray-700 font-semibold text-left"
                      >
        ...
        <Card className="bg-gray-800 border border-gray-700 shadow-lg rounded-lg">
          ...
          <CardContent className="p-6">
            <p className="mb-4 text-gray-300">
              Are you sure you want to reject this request? The fan will be
              refunded.
            </p>
            <textarea
            ...
            <div className="flex justify-end space-x-4">
              <Button
                ...
                variant="outline"
                className="px-4 py-2 text-gray-300 border border-gray-500 rounded-md hover:bg-gray-700 transition duration-200 ease-in-out"
              >
                Cancel
              </Button>
              <Button
